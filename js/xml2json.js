...
function xml2json(xml) {
    ...
    */
    ...
    var md = {
        ...
        data_security_classification: $(xml).find(xpaths.Data_Classification).attr('codeListValue'),
...
function getKeywords(xml) {
    ...
    $(xml).find(xpaths.Data_Keywords).each(function() {
        var kw = {
            ...
            thesaurus_dates: getDates($(this), 'Data_ThesaurusDates'),
        }
        data.push(kw);
    });
    return data;
...
function getUseLimitations(xml) {
    ...
    $(xml).find(xpaths.Data_UseLimitations).each(function() {
        var ul = $(this).find(xpaths.Data_UseLimitation).text();
        data.push(ul);
    });
    return data;
}
...
function getConformities(xml) {
    ...
    $(xml).find(xpaths.Data_DQ_Conformities).each(function() {
        var dc = {
            ...
            dates: getDates($(this), 'Data_DQ_ConformityDates'),
